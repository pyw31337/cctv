...
const RUNTIME_CONFIG = window.CCTV_RUNTIME_CONFIG || {};
...
const APP_BUILD_VERSION = RUNTIME_CONFIG.buildVersion || '20260901-playback1';
...
const QUALITY_CONFIG = window.CCTV_QUALITY_CONFIG || RUNTIME_CONFIG.quality || {};
const QUALITY_TELEMETRY_ENDPOINT = QUALITY_CONFIG.telemetryEndpoint || RUNTIME_CONFIG.qualityTelemetryEndpoint || 'https://cctv-quality.pyw31337.workers.dev/v1/events';
const QUALITY_SUMMARY_URL = QUALITY_CONFIG.summaryUrl || RUNTIME_CONFIG.qualitySummaryUrl || 'https://cctv-quality.pyw31337.workers.dev/v1/summary';
const QUALITY_SUMMARY_FALLBACK_URL = RUNTIME_CONFIG.qualitySummaryFallbackUrl || 'data/quality_summary.json';
const DEFAULT_PROXY_BASES = Array.isArray(RUNTIME_CONFIG.proxyBases) && RUNTIME_CONFIG.proxyBases.length > 0
    ? RUNTIME_CONFIG.proxyBases
    : [
        'https://158.179.194.163.sslip.io',
        'https://cctv-proxy.pyw213.workers.dev'
    ];
const PROXY_BASES = Array.isArray(QUALITY_CONFIG.proxyBases) && QUALITY_CONFIG.proxyBases.length > 0
    ? QUALITY_CONFIG.proxyBases
          .map(base => String(base || '').trim().replace(/\/+$/, ''))
          .filter(Boolean)
    : DEFAULT_PROXY_BASES;
const ORACLE_CANDIDATES = [...new Set(PROXY_BASES.length > 0 ? PROXY_BASES : DEFAULT_PROXY_BASES)];
const PUBLIC_PROXY_BASE = (QUALITY_CONFIG.publicProxyBase || RUNTIME_CONFIG.publicProxyBase || ORACLE_CANDIDATES[0] || DEFAULT_PROXY_BASES[0]).replace(/\/+$/, '');
const WORKER_PROXY_BASE = (QUALITY_CONFIG.workerProxyBase || RUNTIME_CONFIG.workerProxyBase || ORACLE_CANDIDATES.find(base => base !== PUBLIC_PROXY_BASE) || DEFAULT_PROXY_BASES[1]).replace(/\/+$/, '');
function hostFromBase(base, fallback) {
    try {
...
const PUBLIC_PROXY_HOST = hostFromBase(PUBLIC_PROXY_BASE, '158.179.194.163.sslip.io');
const WORKER_PROXY_HOST = hostFromBase(WORKER_PROXY_BASE, 'cctv-proxy.pyw213.workers.dev');
const GITS_PROXY_BASE = (RUNTIME_CONFIG.gitsProxyBase || 'https://cctv-proxy-hoon-001.fly.dev').replace(/\/+$/, '');
const CANARY_STATUS_URL = QUALITY_CONFIG.canaryStatusUrl || RUNTIME_CONFIG.canaryStatusUrl || `${PUBLIC_PROXY_BASE}/canary-status`;
const CANARY_STATUS_FALLBACK_URL = RUNTIME_CONFIG.canaryStatusFallbackUrl || 'data/canary_status.json';
const WORLD_TOUR_DATA_URL = RUNTIME_CONFIG.worldTourDataUrl || `data/world_tour_cams.json?v=${APP_BUILD_VERSION}`;
...
const AUTO_FAILOVER_FALLBACK_WAIT_MS = 2800;
const FALLBACK_PROBE_CONCURRENCY = 4;
...
Object.defineProperty(window, 'JEJU_PROXY_BASE', {
    get: () => `${PUBLIC_PROXY_BASE}/jeju`,
    configurable: true
});

Object.defineProperty(window, 'KB_PROXY_BASE', {
    get: () => `${PUBLIC_PROXY_BASE}/kb`,
    configurable: true
});

Object.defineProperty(window, 'LIVE_HEALTH_STATUS_URL', {
    get: () => QUALITY_CONFIG.healthStatusUrl || `${PUBLIC_PROXY_BASE}/health-status`,
    configurable: true
});
...
const OUTSKIRT_CONTEXT_PATTERN = /(고속|고속도로|서울양양선|수도권제|순환선|도시고속|자동차전용|국도|지방도|IC|JC|TG|영업소|터널|램프|휴게소|졸음쉼터|분기점|진입로|외부|하이패스|대교|교량|고가도로|외곽|삼패|팔당|시외|시계)/i;
...
async function getZ3StreamUrl(cctvip) {
    ...
    return `${WORKER_PROXY_BASE}/z3?url=${encodeURIComponent(tokenUrl)}`;
}
...
function applyQualitySummary(summary) {
    ...
    state.qualitySummary = {
        ...
        browser_canary: summary.browser_canary || null,
...
async function handleSearchInput(e) {
    ...
    try {
        ...
        if (requestToken !== searchRequestToken || $('#search-input').value.trim() !== query) return;
...
async function handleSearchSubmit() {
    ...
    try {
        ...
        // Enter means "go there" for an exact place search. Keep the result
        // list for ambiguous queries so a nearby but wrong place is not opened.
        const normalizedQuery = query.replace(/\s+/g, '').toLowerCase();
        const exact = results.find(item => {
            if (item?.isWorldTourCam) return false;
            const name = String(item?.place_name || '').replace(/\s+/g, '').toLowerCase();
            const address = String(item?.address_name || '').replace(/\s+/g, '').toLowerCase();
            return name === normalizedQuery || name.includes(normalizedQuery) || address.includes(normalizedQuery);
        });
        if (exact && Number.isFinite(Number(exact.y)) && Number.isFinite(Number(exact.x))) {
            selectPlace(exact.y, exact.x, exact.place_name, exact.address_name || '');
            return;
        }
        renderSearchResults(results);
...
function getGitsCameraId(cctv, url = cctv?.directUrl || cctv?.url || '') {
    if (!cctv) return '';
    // Older GITS catalog rows did not persist original_id, although the
    // numeric camera id is present in the signed URL path.
    const match = String(url || '').match(/gitsview\.gg\.go\.kr\/(\d+)(?:\/|$)/i);
    if (match) return match[1];

    return String(cctv.original_id || '').trim();
}
...
function getQualitySummaryAdjustment(cctv) {
    ...
    // Prefer feeds that consistently deliver a useful picture, not merely a
    // feed that happened to answer once. Resolution is intentionally a small
    // ranking factor so a nearby reliable SD camera still beats a distant HD
    // camera.
    const pixelCount = metrics.avgWidth * metrics.avgHeight;
    if (pixelCount > 0 && pixelCount < 320 * 240) adjustment += 1.2;
    else if (pixelCount > 0 && pixelCount < 640 * 360) adjustment += 0.45;
    else if (pixelCount >= 1280 * 720 && metrics.successRate >= 0.85) adjustment -= 0.35;

    if (metrics.samples >= 6 && metrics.successRate >= 0.9 && metrics.avgFirstFrameMs > 0 && metrics.avgFirstFrameMs < 3500) {
        adjustment -= 1;
    }

    const confidence = Math.min(1, Math.max(0.25, metrics.samples / 30));
    return Math.max(-1.2, Math.min(8, adjustment * effective.weight * confidence));
}
...
function getStreamQualityScore(cctv) {
    ...
    if (source === 'JEJU' || url.includes(`${PUBLIC_PROXY_HOST}/jeju`)) {
        score += 0.08;
    }
...
function getRoadContextPriority(cctv, distanceKm) {
    ...
    const looksUrban = URBAN_CONTEXT_PATTERN.test(normalizedName) && !looksOutskirt;
...
function getPlaybackStartupTimeoutMs(cctv, source, url) {
    if (source === 'JEJU' || inferRegionKey(cctv) === 'JEJU') {
        return JEJU_PLAYBACK_STARTUP_TIMEOUT_MS;
    }

    // Direct feeds either respond quickly or are unavailable. Do not block a
    // panel for the old 22-second generic HLS timeout before using a fallback.
    const isDirectHls = /\.m3u8(?:[?#]|$)/i.test(String(url || ''));
    const isFastSource = ['GITS', 'UTIC', 'ITS', 'NOWJEJU'].includes(String(source || '').toUpperCase());
    if (isDirectHls || isFastSource) return 9000;
    return getPlaybackTimeoutMs(cctv);
}
...
function updateNearestCctvs() {
    ...
    const available = ranked.filter(cctv => !isKnownUnavailableCamera(cctv));
    const selectable = available.filter(cctv => CctvVisibilityEngine.isVisibleOnMap(cctv));
...
function dedupeSceneCandidates(candidates, limit = NEAREST_RESULT_LIMIT) {
    ...
    const variantsByScene = new Map();

    // A single physical camera is often published by both UTIC and a local
    // provider. Keep one visible slot for that scene, but retain every source
    // as a playback fallback instead of throwing the duplicate away.
    const mergeVariants = (variants) => {
        if (!variants || variants.length === 0) return null;
        const rank = (item) => {
            const health = item?._health || getCameraHealthMeta(item);
            ...
            const confidence = {
                ...
                DOWN: 8,
                PLAYBACK_ERROR: 20,
                CAMERA_INVESTIGATE: 20,
                CAMERA_CRITICAL: 30,
                UNSUPPORTED: 40
            }[status] ?? 2;
            const quality = Number(item?._streamQuality ?? getStreamQualityScore(item)) || 0;
            const direct = isDirectVideoPlaybackCandidate(item) ? 1 : 0;
            const sourcePreference = item?.source === 'GITS' ? 0.08 : 0;
            return (confidence * 100) - (quality * 10) - (direct * 4) - sourcePreference;
        };

        const primary = variants.slice().sort((a, b) => rank(a) - rank(b))[0];
        const knownUrls = new Set([
            primary.directUrl || primary.url,
            ...(Array.isArray(primary.backup_urls) ? primary.backup_urls.map(item => normalizeBackupEntry(item, primary)?.url) : [])
        ].filter(Boolean));
        const backups = Array.isArray(primary.backup_urls)
            ? primary.backup_urls.map(item => normalizeBackupEntry(item, primary)).filter(Boolean)
            : [];

        variants.forEach(variant => {
            if (!variant || variant.id === primary.id) return;
            const url = variant.directUrl || variant.url;
            if (!url || knownUrls.has(url)) return;
            knownUrls.add(url);
            backups.push({
                id: variant.id,
                source: variant.source || primary.source || '',
                url,
                name: variant.name || primary.name,
        ...
        return backups.length > 0 ? { ...primary, backup_urls: backups } : primary;
    };
    ...
    for (const cctv of candidates || []) {
        ...
        const sceneKey = getCctvSceneKey(cctv);
        if (sceneKey) {
            const variants = variantsByScene.get(sceneKey) || [];
            variants.push(cctv);
            variantsByScene.set(sceneKey, variants);
            if (sceneKeys.has(sceneKey)) continue;
            sceneKeys.add(sceneKey);
        }
        selected.push(sceneKey ? mergeVariants(variantsByScene.get(sceneKey)) : cctv);
        idKeys.add(cctv.id);
        if (selected.length >= limit) break;
    }

    // The ranked list can contain a lower-ranked source variant after the
    // representative was inserted. Refresh representatives with all variants
    // collected before returning the list.
    return selected.map(item => {
        const sceneKey = getCctvSceneKey(item);
        return sceneKey && variantsByScene.has(sceneKey)
            ? mergeVariants(variantsByScene.get(sceneKey))
            : item;
    });
}
...
function renderVideoGrid() {
    ...
    const renderToken = (state.videoGridRenderToken || 0) + 1;
    state.videoGridRenderToken = renderToken;
    ...
    panels.forEach((panel, index) => {
        ...
        if (cctv) {
            const delay = index * 120; // Start all four feeds quickly without a request burst.
            // Warm the verified fallback while the primary feed is loading.
            prepareManualRetryFallback(panel, cctv);
            panel._loadingTimer = setTimeout(() => {
                panel._loadingTimer = null;
                if (state.videoGridRenderToken !== renderToken) return;
...
function getStandbyProbeUrl(candidate) {
    ...
    const kind = getUrlParam(url, 'kind');
    const originalId = getGitsCameraId(candidate, url) || getUrlParam(url, 'cctvip');

    if (source === 'GITS' && originalId) {
        return `${GITS_PROXY_BASE}/gits?cctvip=${encodeURIComponent(originalId)}&_t=${Date.now()}`;
    }
...
async function prepareVerifiedManualRetryFallback(panel, sourceCctv) {
    ...
    const promise = (async () => {
        // Probe a small batch in parallel. Sequential probes made a cold
        // fallback search very slow when several public feeds were down.
        for (let offset = 0; offset < candidates.length; offset += FALLBACK_PROBE_CONCURRENCY) {
            const batch = candidates.slice(offset, offset + FALLBACK_PROBE_CONCURRENCY);
            const results = await Promise.all(batch.map(async item => ({
                item,
                ok: await probeStandbyCandidate(item.candidate)
            })));
            const verified = results.find(result => result.ok);
            if (!verified) continue;

            const fallback = verified.item.candidate;
            panel._verifiedRetryFallback = { sourceId: sourceCctv.id, cctv: fallback };
            panel._preparedRetryFallback = { sourceId: sourceCctv.id, cctv: fallback };
...
async function switchToPreparedRetryFallback(panel, sourceCctv, options = {}) {
    ...
    let fallback = panel._verifiedRetryFallback?.sourceId === sourceCctv.id
        ...
        : null;
    if (!fallback) {
        const pending = prepareVerifiedManualRetryFallback(panel, sourceCctv);
...
function createVideoElement(cctv, sourceIndex = 0) {
    ...
    const shouldProxy = url && !url.includes(WORKER_PROXY_HOST) && !url.includes(`${PUBLIC_PROXY_HOST}/proxy`);
    ...
    const genericProxyBase = isRawIpStreamUrl(url)
        ? `${PUBLIC_PROXY_BASE}/proxy`
        : `${WORKER_PROXY_BASE}/proxy`;
    ...
    const isProxy = url.includes('cctv-proxy-hoon-001.fly.dev')
        || url.includes(WORKER_PROXY_HOST)
        || url.includes(`${PUBLIC_PROXY_HOST}/proxy`);
    ...
    if (isGits && getGitsCameraId(cctv, originalPlaybackUrl)) {
        const video = document.createElement('video');
        video.style.cssText = 'width:100%;height:100%;object-fit:cover;object-position:center center;';
        if (is43) video.dataset.aspectRatio = '4:3';
        video.muted = true;
        ...
        const gitsId = getGitsCameraId(cctv, originalPlaybackUrl);
        const gitsUrl = `${GITS_PROXY_BASE}/gits?cctvip=${encodeURIComponent(gitsId)}&_t=${Date.now()}`;
        const failGits = () => {
        ...
        (async () => {
            ...
            try {
                if (isZ3 && z3CctvIp) {
                        ...
                        try {
                            ...
                            if (idParam) {
                                ...
                                const z3Resp = await fetch(
                                    `${WORKER_PROXY_BASE}/z3?url=${encodeURIComponent(tokenUrl)}`,
                                    { cache: 'no-store' }
                                );
                                if (z3Resp.ok) {
                    ...
                    const workerUrl = `${WORKER_PROXY_BASE}/utic?${uticSearch}&_t=${Date.now()}`;
    ...
    if ((isHls || isSecureStream || isProxy || isKnownHlsSource) && Hls.isSupported()) {
        ...
        const hlsStartupTimeoutMs = getPlaybackStartupTimeoutMs(cctv, selectedSource, url);
...
function handleStreamFailover(wrapper, cctv, nextIndex) {
    ...
    if (isRetryingPrimary || nextIndex <= backupCount) {
        ...
        // Do not wait for a manual retry after all known URLs fail. The
        // verified fallback is warmed during initial loading, so this usually
        // switches immediately; the bounded wait covers a cold cache.
        ...
        const autoSwitch = async () => {
            const switched = panel
                ? await switchToPreparedRetryFallback(panel, cctv, {
                    requireVerified: true,
                    waitMs: AUTO_FAILOVER_FALLBACK_WAIT_MS
                })
                : false;
            if (switched) return;

            // Keep the existing geofenced controller as the second line of
            // defence when no probe has succeeded yet.
            if (currentCount >= 1) {
                const next = findAnotherNearbyCctv(cctv);
                if (next) {
            ...
            const errPh = createErrorPlaceholder({
                ...
                detail: '주변의 정상 송출 카메라를 찾지 못했습니다. 잠시 후 다시 시도해 주세요.',
                retryLabel: `다시 시도 (${Math.min(MANUAL_RETRY_PRIMARY_ATTEMPTS, currentCount + 1)}/${MANUAL_RETRY_PRIMARY_ATTEMPTS})`,
                retryFn: currentCount < MANUAL_RETRY_PRIMARY_ATTEMPTS
                    ? async () => {
                ...
                onTryAnother: async () => {
                    const next = findAnotherNearbyCctv(cctv);
                    if (next) switchCctvTarget(wrapper, next);
                },
                cctv
            });
            wrapper.appendChild(errPh);
        };
        setTimeout(() => { autoSwitch().catch(error => console.warn('[Auto-Failover] fallback failed:', error)); }, 80);
        return;
    }
}
...
function getWorldTourEmbedUrl(cam) {
    if (cam.playUrl) return getWorldTourPlayableStreamUrl(cam.playUrl);
    if (cam.embedUrl && !isWorldTourEmbedBlocked(cam, cam.embedUrl)) return getWorldTourPlayableStreamUrl(cam.embedUrl);
...
function getWorldTourPlayableStreamUrl(url) {
    const value = String(url || '').trim();
    if (/^http:\/\//i.test(value) && isWorldTourHlsUrl(value)) return proxyWithOracle(value);
    return value;
}
...
const WORLD_TOUR_MODULE_CONTEXT = {
    state,
    cleanupDomesticVideoGrid,
    loadWorldTourCams,
...
let worldTourModulePromise = null;
async function loadWorldTourModule() {
    if (!worldTourModulePromise) {
        const moduleUrl = new URL(`js/world-tour.js?v=${encodeURIComponent(APP_BUILD_VERSION)}`, window.location.href).href;
        worldTourModulePromise = import(moduleUrl)
            .then(mod => mod.createWorldTourApi(WORLD_TOUR_MODULE_CONTEXT))
            .catch(error => {
                worldTourModulePromise = null;
                throw error;
            });
    }
    return worldTourModulePromise;
}

async function renderWorldTourCams(...args) {
    const api = await loadWorldTourModule();
    return api.renderWorldTourCams(...args);
}

function switchWorldTourViewMode(viewMode) {
    loadWorldTourModule()
        .then(api => api.switchWorldTourViewMode(viewMode))
        .catch(error => console.warn('[WorldTour] lazy-load failed:', error));
}

function focusWorldTourCamOnMap(cam) {
    loadWorldTourModule()
        .then(api => api.focusWorldTourCamOnMap(cam))
        .catch(error => console.warn('[WorldTour] lazy-load failed:', error));
}

function selectWorldTourCam(camId, source = 'card') {
    loadWorldTourModule()
        .then(api => api.selectWorldTourCam(camId, source))
        .catch(error => console.warn('[WorldTour] lazy-load failed:', error));
}

function cleanupWorldTourVideoPlayers(root = document) {
    loadWorldTourModule()
        .then(api => api.cleanupWorldTourVideoPlayers(root))
        .catch(error => console.warn('[WorldTour] cleanup failed:', error));
}

function destroyWorldTourMap() {
    loadWorldTourModule()
        .then(api => api.destroyWorldTourMap())
        .catch(error => console.warn('[WorldTour] map cleanup failed:', error));
}

function updateWorldTourHeaderSwitch(cam) {
    loadWorldTourModule()
        .then(api => api.updateWorldTourHeaderSwitch(cam))
        .catch(error => console.warn('[WorldTour] header update failed:', error));
}
